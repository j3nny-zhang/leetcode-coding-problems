...
public:
    vector<int> findMinHeightTrees(int n, vector<vector<int>>& edges) {
        ...
        // we want to queue in all leaf nodes, those with outdeg 1
        queue<int> q;

        for (int i = 0; i < n; i++) {
            if (outdeg[i] == 1) q.push(i);
        }
        ...
        while(!q.empty()) {
            int size = q.size();
            n -= size; // keep track of how many nodes are left to discover
            ...
            while(size-- > 0) {
                ...
                for (int neighbour : tree[curr]) {
                    if (--outdeg[neighbour] == 1) q.push(neighbour);
                } 
            }
        }
