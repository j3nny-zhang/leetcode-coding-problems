...
public:
    int openLock(vector<string>& deadends, string target) {
        ...
        // put deadends into a set for O(1) retrieval time
        for (string de : deadends) visited.insert(de);

        // base case
        if (visited.find("0000") != visited.end()) return -1;

        // use bfs for shortest path, level pruning
        queue<string> q;
        q.push("0000");
        visited.insert("0000");
        ...
        while (!q.empty()) {
            int level = q.size();
            while (level--) {
                string combination = q.front();
                q.pop();

                if (combination == target) return turns;

                for (int i = 0; i < 4; i++) { // iterate through the wheels
                    for (int dir : {1, -1}) {
                        int num = (combination[i] - '0' + dir + 10) % 10;
                        string newCombination = combination;
                        newCombination[i] = num + '0';

                        if (visited.find(newCombination) == visited.end() && newCombination.size() == 4) {
