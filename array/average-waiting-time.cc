...
public:
    double averageWaitingTime(vector<vector<int>>& customers) {
        int prepStart = customers[0][0];
        double currTime = 0;

        for (vector<int> customer : customers) {
            prepStart = max(prepStart, customer[0]) + customer[1]; // pick customer[0] as the start time if that start time is after everything else has finished
            currTime += (prepStart) - customer[0];
        }

        return currTime / customers.size();
    }
};
