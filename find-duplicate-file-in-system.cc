...
public:
    vector<vector<string>> findDuplicate(vector<string>& paths) {
        unordered_map<string, vector<string>> m; // {content, list of directories}
        
        // split by space to get file dir and contents
        for (string& path : paths) {
            ...
            getline(ss, dir, ' '); // get the directory path
            while (getline(ss, filename, ' ')) {
                string content = filename.substr(filename.find('(') + 1, filename.find(')') - (filename.find('(') + 1));
                string name = dir + "/" + filename.substr(0, filename.find('('));
